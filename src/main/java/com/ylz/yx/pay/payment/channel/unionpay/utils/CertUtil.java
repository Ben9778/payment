...
import static com.ylz.yx.pay.payment.channel.unionpay.utils.SDKUtil.isEmpty;
...
public class CertUtil {
    ...
    /** 5.0磁道加密公钥 */
    private static final PublicKey encryptTrackKey = null;
	/** 签名私钥map：证书路径，私钥 */
	private final static Map<String, Cert> signCerts = new ConcurrentHashMap<String, Cert>();
    /** 5.0接口验签证书Map：certId，公钥 */
    private static final Map<String, PublicKey> verifyCerts = new ConcurrentHashMap<String, PublicKey>();
    /** 加密证书  */
    private static Cert encryptCert = null;
    /** 5.1接口验签证书Map：证书完整字符串，公钥 */
    private static final Map<String, PublicKey> verifyCerts510 = new ConcurrentHashMap<String, PublicKey>();
    /** 6.0统一支付加密pin用，其他接口请勿使用 */
    private static final Cert pinEncryptCert = null;
	 ...
	 */
	private static void initMiddleCert(String path) {
        if(isEmpty(path)){
            logger.warn("中级证书 is empty");
            return;
        }
	 ...
	 */
	private static void initRootCert(String path) {
        if(isEmpty(path)){
            logger.warn("根证书 is empty");
            return;
        }
	 ...
	 * @return
	 */
    protected static Cert getEncryptCert(String path) {
    	if(CertUtil.encryptCert == null) {
            initEncryptCert(path);
    	}
	 ...
	 * @return
	 */
    private static X509Certificate getMiddleCert(String path) {
        if (isEmpty(path)) {
     ...
     * @return
     */
    private static X509Certificate getRootCert(String path) {
        if (isEmpty(path)) {
    ...
    public static PublicKey verifyAndGetVerifyPubKey(String x509CertString, String middleCertPath, String rootCertPath, boolean isIfValidateCNName){
        ...
        // 验证证书链
        if (!CertUtil.verifyCertificate(x509Cert, middleCertPath, rootCertPath, isIfValidateCNName)) {
     ...
     * @return
     */
    private static boolean verifyCertificate(X509Certificate cert, String middleCertPath, String rootCertPath, boolean isIfValidateCNName) {
		...
		try {
			...
			if(!verifyCertificateChain(cert, CertUtil.getMiddleCert(middleCertPath), CertUtil.getRootCert(rootCertPath))){
				return false;
			}
		} catch (Exception e) {
