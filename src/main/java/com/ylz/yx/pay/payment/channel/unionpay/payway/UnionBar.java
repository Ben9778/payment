...
import com.ylz.yx.pay.payment.rqrs.payorder.payway.UnionBarOrderRQ;
import com.ylz.yx.pay.payment.rqrs.payorder.payway.UnionBarOrderRS;
 ...
 * 银联 银联条码支付
 */
@Service("unionPaymentByUnionBarService") //Service Name需保持全局唯一性
public class UnionBar extends UnionpayPaymentService {

    private static final Logger log = new Logger(UnionBar.class.getName());

    @Override
    public String preCheck(UnifiedOrderRQ rq, PayZfdd00 payZfdd00) {

        UnionBarOrderRQ bizRQ = (UnionBarOrderRQ) rq;
    ...
    public AbstractRS pay(UnifiedOrderRQ rq, PayZfdd00 payZfdd00, MchAppConfigContext mchAppConfigContext) throws Exception {
        ...
        UnionBarOrderRQ bizRQ = (UnionBarOrderRQ) rq;//请求上游api参数
        UnionBarOrderRS res = ApiResBuilder.buildSuccess(UnionBarOrderRS.class);//上游api响应结果
        ChannelRetMsg channelRetMsg = new ChannelRetMsg();//响应信息包装
        ...
        contentData.put("qrNo", bizRQ.getAuthCode().trim());    //付款码： C2B码,1-20位数字
        contentData.put("termId", "");                  	    //终端号
